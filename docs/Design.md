...
intermediate objects or final outputs it needs. The scheduler refines work
subjects, records dependencies, answers requests, and resubmits affected work
when compile-time execution or generated code changes the program state.

The source of truth is the canonical AST plus attached semantic state.
...
final output; it must not change language semantics.
...
target output. When work discovers a new dependency, it submits another
...
flowchart LR
    ...
    TypeEngine -->|GenericWork| CompilerWorkScheduler
    CompilerWorkScheduler -->|ComptimeTypedAst| HirLowering
...
| `CompilerWorkScheduler` | Owns request scheduling, dependency ordering, typed storage ids, and answer delivery. It may use a stack internally, but it is not a renamed pipeline. |
| `RequestState` | Assigns `RequestId`s, tracks blockers, and maps answers back to AST nodes. It can live inside `CompilerWorkScheduler`. |
| `TypeEngine` | Types AST scopes, records constraints, and reports `TypeNeed`, `GenericWork`, or `CompileTimeNeed` after a whole scope typing pass. |
...
| `NativeEmitter` | Emits target-native object code, assembly, or backend-specific outputs. |

## Requests And Comptime

A compile-time need is any value, type, declaration, code fragment, or
identity-forming argument that must be known before the current work can
continue.
...
If a type is unknown because comptime must produce it, the requested work still
uses typed AST, HIR, MIR, LIR, and `ExecutionEngine`; after the answer is
applied, the blocked AST scope is typed again.
...
invalidates only the affected typed, lowered, executed, and emitted objects.
...
encoded in the fully qualified path, dependency key, cache key, and typed
lowering storage ids. The scheduler handles this by resolving the affected work
subject to a concrete path, not by forking a new language mode.

## Scoped Lowering

Scoped lowering is the shared `typed AST -> HIR -> MIR -> LIR` path for a
specific function, item, block, expression, const body, or generated fragment.
It is gradual: the compiler lowers the smallest work subject demanded by a
consumer, caches the result, and records dependencies.
...
Mode branching should happen at the requested output boundary. Earlier
...
Async support should be implemented through lowered executable LIR, not
...
Saving intermediates should write the typed stored objects or emitted outputs
that actually exist for the requested work:

- `.ast` for canonical AST state;
- `.ast-typed` for typed AST annotations;
- `.hir`, `.mir`, and `.lir` for scoped lowered storage;
...
- HIR, MIR, and LIR storage for affected scopes;
...
1. Define concrete `CompilerWorkScheduler`, request state, dependency graph,
   and typed object store APIs.
...
3. Align interpreter mode with lowered execution so it stops owning separate
   AST-only semantics.
4. Define request identity rules for work subjects, inferred generics,
   explicit comptime arguments, generated declarations, and AST-producing
   comptime results.
