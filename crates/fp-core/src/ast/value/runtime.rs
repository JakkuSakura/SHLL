...
impl RuntimeValue {
        ...
        F: FnOnce(&mut Value) -> Result<()>,
    {
        match self {
            ...
            RuntimeValue::SharedAtomic(arc) => {
                let mut borrowed = arc.try_write().map_err(|_| "Borrow check failed")?;
                f(&mut borrowed)
            }
            RuntimeValue::Extension(ext) => {
    ...
    pub fn take_ownership(self) -> Result<Value> {
        match self {
            ...
            RuntimeValue::Borrowed { value, .. } => Ok(value),
            RuntimeValue::BorrowedMut { value, .. } => Ok(value),
            RuntimeValue::Shared(rc) => Ok(Rc::try_unwrap(rc)
                .map_err(|_| "Borrow check failed")?
                .into_inner()),
            RuntimeValue::SharedAtomic(arc) => Ok(Arc::try_unwrap(arc)
                .map_err(|_| "Borrow check failed")?
                .into_inner()
                .map_err(|_| "Borrow check failed")?),
    ...
    pub fn is_borrowed(&self) -> bool {
        matches!(
            self,
            RuntimeValue::Borrowed { .. } | RuntimeValue::BorrowedMut { .. }
        )
    }
    ...
    pub fn is_shared(&self) -> bool {
        matches!(
            self,
            RuntimeValue::Shared(_) | RuntimeValue::SharedAtomic(_)
        )
    }
