...
impl RuntimePass for LiteralRuntimePass {
    ...
    fn assign(&self, _target: &AstExpr, _value: RuntimeValue, _ctx: &SharedScopedContext) -> Result<()> {
        // Literal runtime doesn't support assignment
        bail!("Assignment not supported in literal runtime")
    }
    
    fn call_method(&self, obj: RuntimeValue, method: &str, _args: Vec<RuntimeValue>) -> Result<RuntimeValue> {
        // Basic method support for literals
        match method {
            "clone" => Ok(RuntimeValue::literal(obj.get_value())),
            "to_string" => Ok(RuntimeValue::literal(AstValue::string(format!("{}", obj.get_value())))),
