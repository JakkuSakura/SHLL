...
pub struct RustRuntimePass {
    ...
    /// Track borrowed variables (by name)
    borrowed_values: std::collections::HashSet<String>,
}
...
impl RustRuntimePass {
    ...
    pub fn new() -> Self {
        Self {
            ...
            borrowed_values: std::collections::HashSet::new(),
        }
    }
    ...
    fn is_borrowed(&self, var_name: &str) -> bool {
        self.borrowed_values.contains(var_name)
    }
    /// Add a borrow
    fn add_borrow(&mut self, var_name: String) {
        self.borrowed_values.insert(var_name);
    }
    // Remove a borrow
...
impl RustRuntimePass {
    ...
    pub fn borrow_value_mut(
        ...
        if self.is_borrowed(&var_name) {
            bail!(
                "Cannot create mutable borrow: '{}' is already borrowed",
                var_name
            );
        }
