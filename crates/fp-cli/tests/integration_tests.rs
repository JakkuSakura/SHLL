...
fn test_cli_init_basic() {
    ...
    assert!(temp_dir.path().join(project_name).join("Ferrophase.toml").exists());
    assert!(temp_dir.path().join(project_name).join("src").join("main.fp").exists());
...
fn test_cli_eval_simple() {
    ...
    cmd.assert()
        .code(predicate::in_iter([0, 1])); // Accept success or failure for now
}
