...
"#;
    ...
    assert!(
        output_content.contains("interface Point"),
        "Should contain Point interface"
    );
    assert!(
        output_content.contains("interface Color"),
        "Should contain Color interface"
    );

    // Check for field definitions
    assert!(
        output_content.contains("x: number"),
        "Point should have x field"
    );
    assert!(
        output_content.contains("y: number"),
        "Point should have y field"
    );
    assert!(
        output_content.contains("r: number"),
        "Color should have r field"
    );
    assert!(
        output_content.contains("g: number"),
        "Color should have g field"
    );
    assert!(
        output_content.contains("b: number"),
        "Color should have b field"
    );

    // Check for main function
    assert!(
        output_content.contains("function main()"),
...
"#;
    ...
    assert!(
        output_content.contains("function createPoint"),
        "Should contain Point factory function"
    );
...
"#;
    ...
    assert!(
        types_content.contains("interface User"),
    ...
    assert!(
        types_content.contains("name: string"),
        "User should have name field"
    );
    assert!(
        types_content.contains("active: boolean"),
        "User should have active field"
    );
}
...
async fn test_transpile_multiple_files() {
    ...
    fs::write(
        &input_file2,
        "struct Color { r: u8, g: u8, b: u8, } fn main() {}",
    )
    .unwrap();
    ...
    assert!(
        output_file2.exists(),
        "Second output file should be created"
    );
}
