...
fn print_version_info() {
    ...
    println!("Built: {}", option_env!("VERGEN_BUILD_TIMESTAMP").unwrap_or("unknown"));
    println!("Commit: {}", option_env!("VERGEN_GIT_SHA").unwrap_or("unknown"));
    println!();
}
...
fn print_build_info() {
    ...
    println!("Features: {}", option_env!("VERGEN_CARGO_FEATURES").unwrap_or("default"));
    println!();
}
...
fn print_feature_info() {
    ...
    let features = vec![
        ("Multi-language frontend", "Rust-with-extensions, Python, JavaScript"),
        ("Const evaluation", "Advanced compile-time computation"),
        ("Optimization passes", "Specialization, inlining, const folding"),
        ("Backend targets", "Rust transpilation, LLVM, WebAssembly"),
        ("Type system", "Structural typing, unions, intersections"),
        ("Metaprogramming", "Compile-time intrinsics and code generation"),
    ];
...
fn print_basic_info() {
    ...
    println!();
    println!("Documentation: {}", style("https://github.com/your-org/FerroPhase").cyan());
    println!("Issues: {}", style("https://github.com/your-org/FerroPhase/issues").cyan());
    println!();
