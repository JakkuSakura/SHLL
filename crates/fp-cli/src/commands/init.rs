...
pub async fn init_command(args: InitArgs, config: &CliConfig) -> Result<()> {
    ...
    match args.template.as_str() {
        ...
        "multi-lang" => {
            create_multi_lang_template(&project_dir, &args.project_name, config).await?
        }
        _ => {
...
async fn create_binary_template(
    project_dir: &Path,
    project_name: &str,
    config: &CliConfig,
) -> Result<()> {
...
async fn create_multi_lang_template(
    project_dir: &Path,
    project_name: &str,
    config: &CliConfig,
) -> Result<()> {
    ...
    let main_content = format!(
        r#"//! {} - Multi-language FerroPhase project
...
fn print_success_message(project_name: &str, project_dir: &Path) {
    println!();
    println!(
        "{} Successfully created FerroPhase project '{}'",
        style("✓").green(),
    ...
    println!(
        "  {} https://github.com/your-org/FerroPhase",
        style("📖").cyan()
    );
    println!();
}
