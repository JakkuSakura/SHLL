...
impl FileUtils {
    ...
    pub fn ensure_dir_exists(dir: &Path) -> Result<()> {
        if !dir.exists() {
            std::fs::create_dir_all(dir).map_err(|e| CliError::Io(e))?;
        }
        Ok(())
    }
    ...
    pub fn copy_file(src: &Path, dst: &Path) -> Result<()> {
        ...
        std::fs::copy(src, dst).map_err(|e| CliError::Io(e))?;
    ...
    pub fn write_file(path: &Path, content: &str) -> Result<()> {
        ...
        std::fs::write(path, content).map_err(|e| CliError::Io(e))?;
    ...
    pub fn modification_time(path: &Path) -> Result<std::time::SystemTime> {
        let metadata = std::fs::metadata(path).map_err(|e| CliError::Io(e))?;

        metadata.modified().map_err(|e| CliError::Io(e))
    }
...
mod tests {
    ...
    fn test_find_files_with_patterns() {
        ...
        // Find only .fp files
        let files = FileUtils::find_files(root, &["**/*.fp".to_string()], &[]).unwrap();
        assert_eq!(files.len(), 2);
