...
impl Process {
    ...
    pub fn args(self, extra_args: ::alloc::Vec<&str>) -> Process {
    ...
    pub fn output_result(self) -> ProcessResult {
        ...
        let mocked: ::core::option::Option<::std::intrinsics::test::CommandMockMatch> =
            ::std::test::apply_command_mock(&rendered_command);
        match mocked {
            ::core::option::Option::Some(mocked) => ProcessResult {
                stdout: mocked.stdout,
                stderr: mocked.stderr,
                status: mocked.status,
            },
            ::core::option::Option::None => ProcessResult {
                stdout: "",
                stderr: "",
...
impl Command {
    ...
    pub fn args(self, extra_args: ::alloc::Vec<&str>) -> Command {
        Command {
...
pub fn run_argv(program: &str, args: ::alloc::Vec<&str>) {
    Process::new(program).args(args).run()
}

pub fn ok_argv(program: &str, args: ::alloc::Vec<&str>) -> bool {
    Process::new(program).args(args).ok()
}

pub fn output_argv(program: &str, args: ::alloc::Vec<&str>) -> str {
    Process::new(program).args(args).output()
}

pub fn status_argv(program: &str, args: ::alloc::Vec<&str>) -> i64 {
    Process::new(program).args(args).status()
}

pub fn run_argv_in(program: &str, args: ::alloc::Vec<&str>, cwd: &str) {
    Process::new(program).args(args).current_dir(cwd).run()
}

pub fn ok_argv_in(program: &str, args: ::alloc::Vec<&str>, cwd: &str) -> bool {
    Process::new(program).args(args).current_dir(cwd).ok()
}

pub fn output_argv_in(program: &str, args: ::alloc::Vec<&str>, cwd: &str) -> str {
    Process::new(program).args(args).current_dir(cwd).output()
}

pub fn status_argv_in(program: &str, args: ::alloc::Vec<&str>, cwd: &str) -> i64 {
...
fn render_argv_command(program: &str, args: ::alloc::Vec<&str>) -> str {
    let mut parts: ::alloc::Vec<str> = ::alloc::Vec::new();
