...
impl<T> Vec<T> {
    ...
    pub fn pop(&mut self) -> ::core::option::Option<T> {
        if self.len == 0usize {
            ...
            self.len = self.len - 1usize;
            ::core::option::Option::Some(self[self.len])
        }
    }

    pub fn get(&self, index: usize) -> ::core::option::Option<T> {
        if index < self.len {
            ::core::option::Option::Some(self[index])
        } else {
            ::core::option::Option::None
        }
    }
...
impl Vec<&str> {
    pub fn join(&self, sep: &str) -> ::alloc::string::String {
        let len = self.len();
