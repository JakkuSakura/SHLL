...
impl fp_core::backend::TargetBackend for BytecodeBackend {
    fn compile_package(
        ...
        let bytecode = lower_program(&mir).map_err(|e| fp_core::error::Error::from(e.to_string()))?;
        ...
        if self.save_intermediates || wants_text {
            let rendered = format_program(&bytecode);
        ...
        if !wants_text || self.save_intermediates {
            let bytes = encode_file(&bytecode).map_err(|e| fp_core::error::Error::from(e.to_string()))?;
