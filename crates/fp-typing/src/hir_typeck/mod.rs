...
pub struct HirTypeChecker {
    /// Stable view of every dependency published when type checking started,
    /// plus the current package's shared in-progress handle. Cloning this view
    /// never clones package data.
    ...
    /// Impl candidates currently being evaluated for an associated-type
    /// projection.  Projection keys alone are insufficient for cycles that
    /// alternate between different target types before returning to the same
    /// impl declaration; the impl's stable HIR owner closes that cycle.
    resolving_impl_projections: HashSet<(hir::HirId, hir::Symbol)>,
    /// Generic associated-type projections currently being expanded.  This
    /// is separate from `resolving_assoc_projections` because `path_ty`
    ...
    /// impl evaluation terminates before the Rust stack is exhausted.
    resolving_impl_headers: HashSet<hir::DefId>,
    resolving_impl_assoc_types: Vec<hir::HirId>,
    ...
    /// Diagnostics are emitted from cloned candidate scopes. Share the
...
impl HirTypeChecker {
    ...
    /// Builds a stable-membership program view around the caller-owned package
    /// handle. The caller remains responsible for publishing that same handle
    ...
    pub fn new(
        ...
        current_package_handle.borrow_mut().index_derived_lookups();
        let mut program = dependency_program.as_deref().cloned().unwrap_or_default();
        program.add_package(current_package_handle.clone());
        ...
        for (index, parameter) in generics.params.iter().enumerate() {
            if matches!(parameter.kind, hir::GenericParamKind::Type { .. }) {
                ...
                if !parameter.bounds.is_empty() {
                    ...
                    // Keep the parameter's own bounds under its real
                    // identity as well.  The name-keyed table is needed by
                    // unresolved `T::Assoc` paths, but associated-constant
                    // lookup has already resolved `T` to this exact
                    // `ParamTy`; using that identity avoids confusing
                    // shadowed parameters with the same spelling.
                    child
                ...
                while let Some(trait_id) = trait_ids.pop() {
                    ...
                    let Some(item) = child.program.item(trait_id) else {
                        continue;
                    };
    ...
    /// Cloned handle to the stable program view used by this checker.
    ...
    async fn request_comptime(
        ...
        let value = resolver(self.program.clone(), request).await?;
    ...
    fn record_error(&self, message: impl Into<String>) {
        ...
        if !self.try_claim_diagnostic(&message, None, None) {
            return;
        }
        self.package()
            .diagnostics
    ...
    fn record_error_with_span(&self, message: impl Into<String>, span: fp_core::span::Span) {
        let message = message.into();
        if !self.try_claim_diagnostic(&message, Some(&span), None) {
            return;
        }
        self.package()
            .diagnostics
    ...
    fn record_item_check_failure(&self, message: impl Into<String>) {
        let message = message.into();
        if !self.try_claim_diagnostic(&message, None, Some(ITEM_CHECK_FAILURE_CODE)) {
            return;
        }
    ...
    fn try_claim_diagnostic(
        ...
        let key = format!(
            ...
            span.map(|value| format!("{value:?}")).unwrap_or_default(),
            code.unwrap_or_default(),
            message
        );
        self.emitted_diagnostic_ids.borrow_mut().insert(key)
    }
}
...
impl HirTypeChecker {
    ...
    async fn impl_assoc_types(
        ...
        let Some(key) = impl_items.first().map(|item| item.hir_id.clone()) else {
            return Ok(HashMap::new());
        };
        if self.resolving_impl_assoc_types.contains(&key) {
            return Err(Error::from("recursive associated-type evaluation"));
        }
        self.resolving_impl_assoc_types.push(key.clone());
        ...
        .await;
        self.resolving_impl_assoc_types.pop();
        result
    }

    fn check_item<'a>(&'a mut self, item: &'a hir::Item) -> crate::BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            // Use the resolved definition identity as the diagnostic owner.
            // Candidate scopes are cloned below, so source text alone cannot
            // distinguish repeated reports from distinct items.
            self.current_item_path = Some(item.def_id.to_string());
            match &item.kind {
    ...
    /// Check a trait declaration in its defining generic context. Method
    /// signatures and associated-item metadata are checked here; default
    /// method bodies are checked when their containing item is type-checked.
    async fn check_trait(
        &mut self,
        trait_def_id: hir::DefId,
        trait_def: &hir::Trait,
    ) -> Result<()> {
        let mut scope = self.with_generics(&trait_def.generics).with_self_type(Ty {
            kind: TyKind::Param(ty::ParamTy {
                index: u32::MAX,
                name: hir::Symbol::new("Self"),
            }),
        });
        let self_param = ty::ParamTy {
            index: u32::MAX,
            name: hir::Symbol::new("Self"),
        };
        let self_bound_hir_id = trait_def
            .generics
            .params
            .first()
            .map(|parameter| parameter.hir_id.clone())
            .unwrap_or_else(|| hir::HirId::new(hir::OwnerId(trait_def_id.clone()), 0));
        scope
            .projection_param_bounds
            .entry(self_param)
            .or_default()
        ...
        for item in &trait_def.items {
            match &item.kind {
                hir::TraitItemKind::Method(function) => {
                    let mut method_scope = scope.with_generics(&function.sig.generics);
                    method_scope.check_signature(&function.sig).await?;
                }
    ...
    fn check_expr<'a>(&'a mut self, expr: &'a hir::Expr) -> crate::BoxFuture<'a, Result<Ty>> {
        Box::pin(async move {
            let ty = self.check_expr_inner(expr).await?;
            self.package()
    ...
    fn check_expr_inner<'a>(&'a mut self, expr: &'a hir::Expr) -> crate::BoxFuture<'a, Result<Ty>> {
        match &expr.kind {
            ...
            hir::ExprKind::Path(path) => Box::pin(async move { self.expr_path_ty(path).await }),
            hir::ExprKind::Binary(op, lhs, rhs) => self.check_binary_expr(op, lhs, rhs, expr.span),
            hir::ExprKind::Unary(op, value) => self.check_unary_expr(op, value, expr.span),
            hir::ExprKind::Reference(reference) => self.check_reference_expr(reference),
            hir::ExprKind::Call(callee, args) => Box::pin(async {
                let mut callee = callee.clone();
                // AST->HIR keeps the type head on a qualified constructor
                // path when name resolution cannot inspect the owning enum
                // yet.  Complete the value-namespace lookup here, before
                ...
                if let TyKind::Adt(adt, _) = &callee_ty.kind {
                    if adt.flags.contains(AdtFlags::IS_STRUCT) {
                        return Ok::<_, fp_core::error::Error>(callee_ty);
                    }
                }
                ...
                if let (hir::ExprKind::Path(path), TyKind::FnPtr(signature)) =
                ...
                let output = match &callee_ty.kind {
                    TyKind::FnPtr(signature) => {
                        self.substitute_param_map(&signature.binder.value.output, &substitutions)
                    }
                    _ => unreachable!(),
                };
                // A type-relative path is initially resolved to its nominal
                // receiver (`Vec`, `String`, `Result`, ...), not to the
                // inherent associated function selected from that type's
                // impl. Preserve the latter DefId on the call expression so
                // later lowering never has to recover semantics from path
                ...
                Ok::<_, fp_core::error::Error>(output)
            }),
            hir::ExprKind::MethodCall(receiver, method, generic_args, args) => self
            ...
            hir::ExprKind::FieldAccess(receiver, field) => Box::pin(async move {
                let receiver_ty = self.check_expr(receiver).await?;
                self.field_ty(&receiver_ty, field).await
            }),
            hir::ExprKind::Index(receiver, index) => Box::pin(async {
                let receiver_ty = self.check_expr(receiver).await?;
                ...
                let ty = match &receiver_ty.kind {
                        ...
                        if Some(adt.did.clone()) == self.well_known_struct_def_id("HashMap") =>
                    {
                        let (Some(GenericArg::Type(key_ty)), Some(GenericArg::Type(value_ty))) =
                            ...
                            return Ok(self
                                .error_ty("HashMap index requires key and value type arguments"));
                        };
                ...
                Ok::<_, fp_core::error::Error>(ty)
            }),
            hir::ExprKind::Cast(value, target) => Box::pin(async move {
            ...
            hir::ExprKind::Struct(path, fields) => Box::pin(async move {
                ...
                Ok::<_, fp_core::error::Error>(self.substitute_param_map(&ty, &substitutions))
            }),
            ...
            hir::ExprKind::Match(scrutinee, arms) => Box::pin(async move {
                ...
                for arm in arms {
                    ...
                    {
                        Ty {
                            kind: TyKind::Infer(ty::InferTy::FreshTy(arm.body.hir_id.local_id())),
                        }
                    } else {
                        checked_arm_ty
                    };
                    if let Some(result_ty) = &result {
                        let contextual_integer = if result_is_default_integer_literal
                            && matches!(
                                result_ty.kind,
                                TyKind::Infer(_) | TyKind::Int(ty::IntTy::I64)
                            )
                            && matches!(arm_ty.kind, TyKind::Int(_) | TyKind::Uint(_))
                        {
                            Some(arm_ty.clone())
                        } else if arm_is_default_integer_literal
                            && matches!(arm_ty.kind, TyKind::Infer(_) | TyKind::Int(ty::IntTy::I64))
                ...
                Ok::<_, fp_core::error::Error>(
                    result.unwrap_or_else(|| {
                        self.error_ty("match expression requires at least one arm")
                    }),
                )
            }),
            hir::ExprKind::Block(block) => Box::pin(async move {
                self.check_block_with_expected_tail(block, self.expected_expr_type.clone())
                    .await
            }),
            hir::ExprKind::Loop(block) => Box::pin(async move { self.check_block(block).await }),
            hir::ExprKind::ConstBlock(const_block) => Box::pin(async {
                let expected_output = self.expected_expr_type.clone();
                let body_ty = match expected_output.as_ref() {
                    Some(expected) => {
                        self.with_expected_expr_type(expected.clone())
                ...
                if !ty_contains_param(&body_ty) {
                    ...
                    HirTypeChecker::spawn_comptime_task(&self.root_handle(), def_id, request).await;
                }
                Ok::<_, fp_core::error::Error>(body_ty)
            }),
            hir::ExprKind::While(condition, block) => Box::pin(async {
                let condition_ty = self.check_expr(condition).await?;
                self.require_same_at(&condition_ty, &Ty::bool(), expr.span)?;
                Ok::<_, fp_core::error::Error>(self.check_block(block).await?)
            }),
            hir::ExprKind::For(pat, iter, body) => Box::pin(async {
                ...
                Ok::<_, fp_core::error::Error>(self.unit_ty())
            }),
            hir::ExprKind::Array(values) => Box::pin(async move {
                if values.is_empty() {
            ...
            hir::ExprKind::ArrayRepeat { elem, len } => Box::pin(async {
            ...
            hir::ExprKind::Tuple(values) => Box::pin(async move {
            ...
            hir::ExprKind::Assign(lhs, rhs) => Box::pin(async {
                ...
                Ok::<_, fp_core::error::Error>(lhs)
            }),
            hir::ExprKind::Return(value) | hir::ExprKind::Break(value) => Box::pin(async {
                Ok::<_, fp_core::error::Error>(match value.as_ref() {
            ...
            hir::ExprKind::Continue => Box::pin(std::future::ready(Ok(Ty::never()))),
            hir::ExprKind::Let(pattern, target, value) => Box::pin(async move {
                let declared_ty = self.check_type_expr(target).await?;
                let hint = self
                    .program_rc()
                    .take_raw_refinement_hint(target.hir_id.clone());
                let ty = if let Some(value) = value {
                    // An omitted local annotation lowers to `Infer`.
                    // Bind the name to the initializer's resolved type;
                    // retaining the fresh inference variable here loses
                    // its nominal DefId before a later method lookup.
                    let value_ty = self
                        .with_expected_expr_type(declared_ty.clone())
                        .check_expr(value)
                        .await?;
                    if matches!(target.kind, hir::TypeExprKind::Infer) {
                        value_ty
                    } else {
                        self.require_same_at(&declared_ty, &value_ty, expr.span)?;
                        declared_ty.clone()
                    }
                } else {
                    declared_ty
            ...
            hir::ExprKind::Try(value) => Box::pin(async {
                let input_ty = self.check_expr(&value.expr).await?;
                // Do not derive a second trait obligation from an operand
                // that already failed resolution. The operand's diagnostic
                // is the authoritative rustc-style error.
                if ty_contains_error(&input_ty) {
                    return Ok(input_ty);
                }
                    ...
                    && value.finally.is_none()
                {
                    self.assoc_type_for_self(&input_ty, &hir::Symbol::new("Output"))
                        .await?
                        .unwrap_or_else(|| {
                            self.error_ty(format!("type `{input_ty}` does not implement `Try`"))
                        })
                } else {
                    input_ty.clone()
                };
                ...
                Ok::<_, fp_core::error::Error>(result_ty)
            }),
            hir::ExprKind::With(context, body) => Box::pin(async {
                self.check_expr(context).await?;
                Ok::<_, fp_core::error::Error>(self.check_expr(body).await?)
            }),
            hir::ExprKind::Slice(slice) => Box::pin(async {
            ...
            hir::ExprKind::Closure(closure) => Box::pin(async {
            ...
            hir::ExprKind::Query(_) => Box::pin(std::future::ready(Ok(
                self.error_ty("query typing is not implemented")
            ))),
            hir::ExprKind::IntrinsicCall(call) => {
                Box::pin(async move { self.check_intrinsic(call, expr.span).await })
            }
            hir::ExprKind::FormatString(format) => {
                ...
                Box::pin(std::future::ready(Ok(Ty {
                    kind: TyKind::Slice(Box::new(Ty::uint(ty::UintTy::U8))),
                })))
            }
        }
            ...
            if !integer_literal && !float_literal {
                match op {
                    hir::BinOp::And | hir::BinOp::Or => {
                        ...
                        self.require_same_at(&rhs, &Ty::bool(), span)?;
                    }
                    hir::BinOp::Eq
                    | hir::BinOp::Ne
                    | hir::BinOp::Lt
            ...
            Ok(match op {
                ...
                hir::UnOp::Deref => match value_ty.kind {
                    ...
                    kind => {
                        ...
                        self.deref_target(&value_ty).await.unwrap_or_else(|| {
                            self.error_ty("cannot dereference a non-pointer value")
                        })
                    }
                },
    ...
    fn check_reference_expr<'a>(
        &'a mut self,
        reference: &'a hir::ExprReference,
    ) -> crate::BoxFuture<'a, Result<Ty>> {
        Box::pin(async move {
    ...
    fn check_method_call_expr<'a>(
        &'a mut self,
        receiver: &'a hir::Expr,
        method: &'a hir::Symbol,
        generic_args: Option<&'a hir::GenericArgs>,
        args: &'a [hir::CallArg],
        expr_id: hir::HirId,
        span: fp_core::span::Span,
    ) -> crate::BoxFuture<'a, Result<Ty>> {
        Box::pin(async move {
            ...
            let declared_inputs = match self.method_declared_signature(&receiver_ty, method).await {
                Ok(Some(Ty {
                    kind: TyKind::FnPtr(sig),
                ...
                .await
            {
                Ok((method_def_id, generic_args, output)) => {
                    let package = self.package();
                    package.record_method_resolution(expr_id.clone(), method_def_id.clone());
                    if let Some(args) = generic_args {
                        package.record_generic_method_arg(
                            expr_id,
                            GenericCallResolution {
                ...
                Err(error) => Ok(self.error_ty_with_span(error.to_string(), span)),
            }
        })
    }
                ...
                hir::StmtKind::Local(local) => {
                    let ty = match (&local.ty, &local.init) {
                        (Some(annotation), Some(init)) => {
                            ...
                            let coerce_unsized =
                                self.coerce_unsized_wrapper_compatible(&ty, &resolved_init);
    ...
    async fn path_ty(&mut self, path: &hir::Path) -> Result<Ty> {
        ...
        let Some(def_id) = (match path.res {
            ...
            // builtin tag when HIR is constructed directly (and when a
            // cached/downstream path has not gone through the ordinary
            // type-name resolver).  Resolve it through the same canonical
            ...
            if path.segments.len() >= 2 {
                ...
                // not recover an ADT from the first segment's spelling: that
                // name-only lookup can choose a different package/re-export
                // than the resolver's `Res::Def`, producing a `TyKind::Adt`
                // whose `DefId` cannot reach the impl index.  This is the
                // same resolver/typeck boundary rustc uses for a qualified
                // path: type checking consumes the resolved definition; it
                // does not invent one from text.
        ...
        let Some(item) = self.program_rc().item(def_id.clone()) else {
            ...
            // broken identity boundary. Returning a name-keyed parameter
            // here would hide the publication error and lose the `DefId`
            // required by nominal type and impl lookup.
            return Ok(self.error_ty(format!(
    ...
    async fn expr_path_ty(&mut self, qpath: &hir::QPath) -> Result<Ty> {
        ...
        let Some(item) = self.program_rc().item(def_id.clone()) else {
            ...
            if let Some((generics, self_ty, _, hir::ImplItemKind::AssocConst(constant))) = found {
                let mut scope = self.with_generics(&generics);
                let self_ty = scope.check_type_expr(&self_ty).await?;
                let mut scope = scope.with_self_type(self_ty);
            ...
            let cross_package_member = self
                ...
                .and_then(|package| {
                    // Methods and associated constants are both impl
                    // members. The method-only index drops constants before
                    // this point, which makes a resolved `Type::CONST` lose
                    // its owner as soon as it crosses a crate boundary.
                    let impl_def_id = package.member_to_owning_item.get(def_id)?;
                    package.def_map.get(impl_def_id).cloned()
                })
                .and_then(|item| {
                    let hir::ItemKind::Impl(impl_item) = &item.kind else {
                        return None;
            ...
            if let Some(enum_item) = matched_enum_item {
                ...
                if let Some(payload) = &variant.payload {
                    ...
                    let payload_ty = match &enum_ty.kind {
                        TyKind::Adt(_, args) => {
                            scope.substitute_params(payload_ty, args, &enum_def.generics.params)
                        }
                        _ => payload_ty,
        ...
        match &item.kind {
            ...
            hir::ItemKind::Const(constant) if !self.package().def_map.contains_key(&def_id) => {
                // A foreign constant is dependency metadata at this point.
                // Its initializer was checked while compiling the defining
                // package; checking it again from a use site would make the
                // current package typecheck (and report diagnostics) for
                // implementation details it does not own. This is the same
                // package boundary rustc uses for dependency metadata: a
                // consumer needs the declared type, not the dependency's
                // body. Same-package constants use the declaration-only arm
                // below for the same reason.
                self.check_type_expr(&constant.ty).await
            }
            hir::ItemKind::Const(constant) => {
                // A constant's value use needs its declaration type, not its
                // initializer's item task.  Querying that task here makes
                // forward references work at the cost of allowing A -> B ->
                // A (and A -> A) executor waits.  Keep this query structural:
                // the declaration is independent of the body, and malformed
                // annotations remain hard errors from `check_type_expr`.
                self.check_type_expr(&constant.ty).await
            }
    ...
    /// Resolve `Self::Variant` using the enclosing ADT identity.  Rustc keeps
    /// the `Self` head as a type-relative path and resolves the variant in the
    /// enum's value namespace; the HIR used here has no separate QPath node,
    /// so this is the corresponding typeck step.  The lookup is entirely by
    /// the concrete enum `DefId`, including dependency packages.
    async fn self_enum_variant_constructor(
        &mut self,
        self_ty: &Ty,
        variant_name: &hir::Symbol,
    ) -> Result<Option<Ty>> {
        let TyKind::Adt(adt, args) = &self_ty.kind else {
            return Ok(None);
        };
        let Some(item) = self.program_rc().item(adt.did.clone()) else {
            return Ok(None);
        };
    ...
    async fn checked_impl_self_ty(
        &mut self,
        impl_def_id: &hir::DefId,
        ...
        if !self.resolving_impl_headers.insert(impl_def_id.clone()) {
            return Err(Error::from(format!(
                "recursive impl self-type evaluation for `{impl_def_id}`"
            )));
        }
        let checked = self.check_type_expr(self_ty).await;
        self.resolving_impl_headers.remove(impl_def_id);
        let checked = checked?;
        self.program_rc()
    ...
    async fn assoc_type_from_generic_param_bounds(
        ...
        if self.resolving_generic_projections.contains(&projection_key) {
            return Ok(None);
        }
        self.resolving_generic_projections
            .push(projection_key.clone());
        let result = async {
            ...
            for bound in &bounds {
                ...
                let mut seen = HashSet::new();
                if let Some(assoc_type) = self.trait_assoc_type(&trait_def, assoc_name, &mut seen) {
                    if !assoc_type.bounds.is_empty() {
        ...
        .await;
        self.resolving_generic_projections.pop();
        result
    }

    /// Resolves `T::CONST` for an abstract `T` from the trait bounds in
    /// scope. The declaration belongs to the trait, not to an arbitrary
    /// concrete impl: an abstract parameter has no concrete impl to search.
    /// This follows the same trait/supertrait walk as associated-type and
    /// method lookup, then instantiates the declaration in the trait's
    /// generic scope with `Self = T`.
    async fn generic_param_bound_assoc_const_type(
        &mut self,
        param: &ty::ParamTy,
        ...
        while let Some(trait_id) = trait_ids.pop() {
            ...
            let Some(trait_item) = program.item(trait_id.clone()) else {
                continue;
            };
            let hir::ItemKind::Trait(trait_def) = &trait_item.kind else {
                continue;
            };

            if let Some(constant) = trait_def.items.iter().find_map(|item| {
                if item.name != *const_name {
                    return None;
                }
                let hir::TraitItemKind::AssocConst(constant) = &item.kind else {
                    return None;
                };
                Some(constant.clone())
            }) {
                ...
                let declared_ty = scope.substitute_param_map(&declared_ty, &substitutions);
                return Ok(Some(declared_ty));
            }
    ...
    async fn trait_qualified_method_signature(
        &mut self,
        trait_id: &hir::DefId,
        method_name: &hir::Symbol,
        self_ty: Ty,
    ) -> Result<Option<Ty>> {
        let Some(item) = self.program_rc().item(trait_id.clone()) else {
            return Ok(None);
        };
        let hir::ItemKind::Trait(trait_def) = item.kind else {
            return Ok(None);
        };
        let Some(function) = trait_def.items.into_iter().find_map(|item| {
            if item.name != *method_name {
                return None;
            }
            match item.kind {
                hir::TraitItemKind::Method(function) => Some(function),
                _ => None,
            }
        }) else {
            return Ok(None);
        ...
        let mut scope = self
            .with_generics(&trait_def.generics)
            .with_self_type(self_ty);
        // `function_signature` is cached only by the declaration HirId.
        // Trait-qualified calls instantiate `Self` from their call site, so
        // reusing that cache would leak an earlier unresolved `Self` into a
        // concrete `Result`/`Option`/user-enum call. Rustc's query key
        // includes the substitution environment; use the equivalent
        // uncached path here.
        Ok(Some(
            scope
                .function_signature_uncached(&function, function.sig.output.hir_id.clone())
                .await?,
        ))
    }
    ...
    async fn generic_param_bound_method_signature(
        ...
        for bound in flattened {
            ...
            while let Some(current_id) = trait_ids.pop() {
                ...
                let Some(item) = self.program_rc().item(current_id) else {
                    continue;
                };
                let hir::ItemKind::Trait(trait_def) = &item.kind else {
                    continue;
    ...
    async fn assoc_type_for_self(
        ...
        if ty_contains_error(target_ty) {
            return Ok(None);
        }
        let key = (obligation_shape_key(target_ty), assoc_name.clone());
        ...
        let candidates: Box<dyn Iterator<Item = hir::Item> + '_> = match receiver_def {
            Some(def_id) => Box::new(program.impls_for_adt(def_id)),
        ...
        let result: Result<Option<Ty>> = 'search: {
            for item in candidates {
                ...
                let candidate_key = (impl_item.self_ty.hir_id.clone(), assoc_name.clone());
                if !self
                    .resolving_impl_projections
                ...
                let candidate_result = async {
                    ...
                    // is not this projection's answer. Rejected candidates
                    // stay local to the search, as in rustc's probe phase.
                    let checked_self_ty = scope
                        .checked_impl_self_ty(&item.def_id, &impl_item.self_ty)
                        .await
                        .map_err(|_| ())?;
                    ...
                    let assoc_types = scope
                        .impl_assoc_types(&impl_item.items)
                        .await
                        .map_err(|_| ())?;
                    Ok(assoc_types
                        .get(assoc_name)
                        .map(|ty| scope.substitute_param_map(ty, &substitutions)))
                }
                .await;
                self.resolving_impl_projections.remove(&candidate_key);
                if let Ok(Some(ty)) = candidate_result {
                    break 'search Ok(Some(ty));
                }
            }
            Ok(None)
        };
    ...
    async fn method_output(
        ...
        for _ in 0..8 {
            if let Some(result) = self
                .method_output_at(&current, method, &current_actuals, explicit_generic_args)
                .await?
            {
                return Ok(result);
            }
    ...
    /// Selects an inherent associated function by its concrete impl-member
    /// identity. Unlike a dot call, an associated function has no receiver
    /// value in its argument list, so it must not pass a synthetic receiver
    /// through `method_output`'s call-signature instantiation. The receiver
    /// type is used exclusively to select the applicable impl; the ordinary
    /// `Call` path already type-checks the function arguments and result.
    async fn associated_function_resolution(
        &mut self,
        receiver_ty: &Ty,
        method: &hir::Symbol,
    ) -> Result<Option<hir::DefId>> {
        ...
        for item in candidates {
            ...
            let Some(member) = impl_item.items.iter().find(|member| {
                member.name == *method
                    && matches!(
                        &member.kind,
                        hir::ImplItemKind::Method(function)
                            if !function.sig.inputs.iter().any(|input| {
                                matches!(
                                    &input.pat.kind,
                                    hir::PatKind::Binding { name, .. } if name.as_str() == "self"
                                )
                            })
                    )
            }) else {
            ...
            if Self::method_receiver_matches(
                ...
                return Ok(Some(member.def_id.clone()));
            }
        }
        Ok(None)
    }
    ...
    /// Tests one rustc method-lookup receiver step. The outer lookup loop has
    /// already performed autoderef, so a value receiver must not match a
    /// reference candidate by silently stripping its reference. The only
    /// reference/value crossing here is autoref into the candidate's declared
    /// `self` type; mutable references may satisfy a shared-reference
    /// candidate, but not the reverse.
    fn method_receiver_matches(
        ...
        if ty_contains_error(candidate) || ty_contains_error(receiver) {
            return false;
        }
        match (&candidate.kind, &receiver.kind) {
            (
                TyKind::Ref(_, candidate_inner, candidate_mutability),
                TyKind::Ref(_, receiver_inner, receiver_mutability),
            ) => {
                let mut trial = substitutions.clone();
                if (*candidate_mutability == ty::Mutability::Not
                    || *candidate_mutability == *receiver_mutability)
                    && Self::ty_shapes_compatible(&candidate_inner.kind, &receiver_inner.kind)
                    && scope
                        .unify_call_types_probe(candidate_inner, receiver_inner, &mut trial)
                        .is_ok()
                {
                    *substitutions = trial;
                    return true;
            ...
            (TyKind::Ref(_, candidate_inner, _), _) => {
                let mut trial = substitutions.clone();
                if Self::ty_shapes_compatible(&candidate_inner.kind, &receiver.kind)
                    && scope
                        .unify_call_types_probe(candidate_inner, receiver, &mut trial)
                        .is_ok()
                {
                    *substitutions = trial;
                    true
            ...
            (_, TyKind::Ref(_, _, _)) => false,
            _ => {
                Self::ty_shapes_compatible(&candidate.kind, &receiver.kind)
                    && scope
                        .unify_call_types_probe(candidate, receiver, substitutions)
                        .is_ok()
            }
        }
    }
    ...
    async fn method_output_at(
        ...
        // Method probing is an obligation computation.  A candidate's
        // self-type or signature can ask for the same method obligation
        // again through an associated projection.  rustc's probe tables
        // recognize that active obligation and return an unresolved result;
        ...
        if !self.resolving_assoc_projections.contains(&key) {
            self.resolving_assoc_projections.push(key.clone());
            let result = self
                .method_output_at_inner(receiver_ty, method, actuals, explicit_generic_args)
                .await;
            self.resolving_assoc_projections.pop();
            return result;
        }
    ...
    async fn method_output_at_inner(
        &mut self,
        receiver_ty: &Ty,
        method: &hir::Symbol,
        actuals: &[Ty],
        ...
        if ty_contains_error(receiver_ty) || actuals.iter().any(ty_contains_error) {
            return Ok(None);
        }
        ...
        for item in candidates {
            ...
            // Member-name filtering must happen before `with_generics`:
            // receiver buckets contain every impl for the type, while only a
            // small subset can provide this method (directly or by default).
            let declares_method = impl_item.items.iter().any(|item| {
                item.name == *method && matches!(&item.kind, hir::ImplItemKind::Method(_))
            });
            let declares_trait_default = if declares_method {
                false
            } else {
                impl_item
                    .trait_ty
            ...
            if !declares_method && !declares_trait_default {
                continue;
            }
            let mut scope = self.with_generics(&impl_item.generics);
            let Ok(checked_self_ty) = scope
            ...
            let self_ty = &checked_self_ty;
            // Candidate selection follows one structural rule for every
            // receiver shape.  The indexed candidate list has already
            // limited ADT candidates to the receiver's definition (plus
            // blanket impls); this gate must still compare nested generic
            // arguments and unresolved `Self`/impl parameters uniformly.
            // In particular, a blanket `impl<I> Trait for I` is not an
            // unconditional match: speculative unification records the
            // receiver substitution and rejects incompatible repeated
            // parameters without emitting diagnostics.
            let mut receiver_substitutions = HashMap::new();
            let matches_receiver = Self::method_receiver_matches(
                &scope,
                self_ty,
                receiver_ty,
            ...
            // The receiver match is also the impl's inference step. In
            // rustc, substitutions established by selecting
            // `impl<B, I, F> Iterator for Map<I, F> where
            // F: FnMut(I::Item) -> B` remain active while its associated
            // types and default methods are instantiated. In particular,
            // the callable bound determines `B` from the closure's output;
            // dropping that map leaves `Iterator::Item` as an unrelated
            // generic parameter and makes a later `.sum()` candidate look
            // unresolved even though its expected result supplies `S`.
                                ...
                                if let Some(expected) = &self.expected_expr_type {
                                    if !ty_contains_error(expected) && ty_contains_param(&result) {
                                            ...
                                            .is_ok()
                                        {
                                            substitutions = trial;
                                            result =
                                                scope.substitute_param_map(&result, &substitutions);
                                        }
                                    }
                                }
                                let args = scope.method_generic_args(
    ...
    async fn deref_target(&mut self, receiver_ty: &Ty) -> Option<Ty> {
        ...
        let program = self.program_rc();
        let candidates: Vec<hir::Item> = program.impls_for_adt(receiver_def).collect();
        for item in &candidates {
            let hir::ItemKind::Impl(impl_item) = &item.kind else {
                continue;
            };
    ...
    async fn enum_struct_payload_type(
        ...
        if matches!(
            self.program_rc()
                .item(adt.did.clone())
                .map(|item| matches!(item.kind, hir::ItemKind::Struct(_))),
            Some(true)
        ) {
            Ok(Some(payload))
        } else {
        ...
        match ty.kind {
            ...
            TyKind::Tuple(fields) => Ty {
                kind: TyKind::Tuple(
                    fields
                        .into_iter()
                        .map(|field| Box::new(self.substitute_params(*field, args, parameters)))
                        .collect(),
                ),
            },
    ...
    */
    ...
    fn coerce_unsized_wrapper_compatible(&self, expected: &Ty, actual: &Ty) -> bool {
        if let (TyKind::RawPtr(expected), TyKind::RawPtr(actual)) = (&expected.kind, &actual.kind) {
        ...
        expected_args
            ...
            .zip(actual_args)
            .any(|(expected, actual)| {
                let (GenericArg::Type(expected), GenericArg::Type(actual)) = (expected, actual)
                else {
                    return false;
                };
                matches!(
    ...
    async fn trait_object_wrapper_unsize_compatible(&mut self, expected: &Ty, actual: &Ty) -> bool {
        let (TyKind::Adt(expected_def, expected_args), TyKind::Adt(actual_def, actual_args)) =
            (&expected.kind, &actual.kind)
        else {
            return false;
        ...
        for (expected_arg, actual_arg) in expected_args.iter().zip(actual_args) {
            ...
            let program = self.program_rc();
            let implements_principal = match &actual_inner.kind {
                // `Box<A> -> Box<dyn Error>` is valid in rustc when the
                // generic source parameter is constrained by `Error` (or a
                // supertrait of it). This is a trait-bound fact, not a
                // general type coercion, so inspect the bound identities
                // rather than accepting every parameter-shaped source.
                TyKind::Param(param) => {
                    let bounds = self
                        .projection_param_bounds(param)
                        .or_else(|| self.generic_param_bounds(&param.name))
                    ...
                    if let Some(bounds) = bounds {
                        fn flatten_bounds<'a>(
                            bound: &'a hir::TypeExpr,
                            output: &mut Vec<&'a hir::TypeExpr>,
                        ) {
                            if let hir::TypeExprKind::TypeBinaryOp(op) = &bound.kind
                                && matches!(op.kind, fp_core::ast::TypeBinaryOpKind::Add)
                            {
                                flatten_bounds(&op.lhs, output);
                                flatten_bounds(&op.rhs, output);
                            } else {
                                output.push(bound);
                            }
                        }
                        let mut flattened = Vec::new();
                        for bound in &bounds {
                            flatten_bounds(bound, &mut flattened);
                        }
                        for bound in flattened {
                    ...
                    let mut seen = HashSet::new();
                    let found = loop {
                        let Some(trait_id) = trait_ids.pop() else {
                            break false;
                        };
                        if !seen.insert(trait_id.clone()) {
                            continue;
                        }
                        if trait_id == principal.def_id {
                            break true;
                        }
