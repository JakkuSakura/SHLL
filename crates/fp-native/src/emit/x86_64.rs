...
fn encode_const_bytes(
    ...
    let _size_of = |ty: &LirType| data_layout.size_of(ty).expect("layout query failed");
...
fn copy_temporaries(
    ...
    let mut selected = CANDIDATES.into_iter().filter(|candidate| {
        !forbidden.iter().any(|location| {
            matches!(location, CopyLocation::Address(address) if address == candidate)
        })
    });
...
fn materialize_copy_address(asm: &mut Assembler, destination: Reg, location: CopyLocation) {
    match location {
