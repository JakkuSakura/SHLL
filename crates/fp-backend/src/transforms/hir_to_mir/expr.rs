...
impl HirToMirLowerer {
    ...
    pub(crate) fn is_result_path(&self, path: &hir::Path) -> bool {
        path.segments()
            .last()
            .map(|segment| segment.ident.as_str() == "Result")
            .unwrap_or(false)
    }
    ...
    pub(super) fn infer_generic_from_type_expr(
        ...
        match &ty_expr.kind {
            hir::TypeExprKind::Path(path) => {
                let variant_enum_def = path.res_ref().as_ref().and_then(|res| {
                    ...
                    .kind
                {
                    ...
                    if let hir::Res::Def(def_id) = &path.res_ref() {
                        ...
                        if !matches_def {
                            if let Some(name) = path.segments().last().map(|seg| seg.ident.as_str())
                            {
    ...
    pub(super) fn struct_name_from_type(&self, ty: &hir::TypeExpr) -> Option<String> {
        match &ty.kind {
            hir::TypeExprKind::Path(path) => path
                .segments()
                .last()
                .map(|seg| String::from(seg.ident.clone())),
