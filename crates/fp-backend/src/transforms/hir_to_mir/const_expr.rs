...
pub(super) fn const_value_from_constant(&self, constant: &mir::Constant) -> Option<mir::ConstValue> {
        ...
        ConstContainerArgs::Map { key_ty, value_ty } => {
            ...
            for element in elements {
                let (key_expr, value_expr) = match &element.kind {
                    ...
                    _ => {
                        self.emit_error(span, "HashMap literal expects entries as [key, value]");
                        return None;
                    }
                };
...
pub(super) fn container_args_from_type_expr(&mut self, ty_expr: &hir::TypeExpr) -> Option<ConstContainerArgs> {
    match &ty_expr.kind {
