...
impl LirToMir {
    ...
    /// `fp-backend`'s `HirToMirLowerer` (via its shared `mir::MirPackage`)
    /// specifically so a downstream consumer with no live `HirToMirLowerer`
    /// (like this one) can look it up.
