...
fn transform_expr_uses_typing_resolved_name_table() -> Result<()> {
    ...
    let mut generator = AstToHirLowerer::new(hir::PackageId(0)).with_resolved_names(resolved_names);
...
fn compile_normalization_runs_during_ast_to_hir_lowering() -> Result<()> {
    ...
    let mut generator = AstToHirLowerer::new(hir::PackageId(0)).with_intrinsic_normalizer(
...
fn test_hir_generator_creation() {
    let generator = AstToHirLowerer::new(hir::PackageId(0));
