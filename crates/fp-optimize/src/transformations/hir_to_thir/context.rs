...
pub(super) struct StructInfo {
    pub(super) adt_def: types::AdtDef,
    pub(super) field_map: HashMap<String, (usize, types::Ty)>,
}
...
impl TypeContext {
    ...
    pub(super) fn register_const(&mut self, def_id: types::DefId, name: &str, ty: types::Ty) {
        self.const_types.insert(def_id, ty);
        self.insert_value_name(name, def_id);
    }

    pub(super) fn declare_struct(&mut self, def_id: types::DefId, name: &str) {
    ...
    pub(super) fn lookup_struct_def_id(&self, name: &str) -> Option<types::DefId> {
        self.struct_names.get(name).copied()
    }

    pub(super) fn lookup_value_def_id(&self, name: &str) -> Option<types::DefId> {
        self.value_names.get(name).copied()
    }

    pub(super) fn lookup_function_signature(&self, def_id: types::DefId) -> Option<&types::FnSig> {
        self.function_signatures.get(&def_id)
    }

    pub(super) fn lookup_const_type(&self, def_id: types::DefId) -> Option<&types::Ty> {
    ...
    pub(super) fn ensure_struct_stub(&mut self, name: &str) -> types::DefId {
