...
impl MirGenerator {
    ...
    pub(super) fn transform_binary_op(&self, op: BinOpKind) -> Result<mir::BinOp> {
        match op {
            ...
            BinOpKind::Mod => Ok(mir::BinOp::Rem),
            BinOpKind::And => Ok(mir::BinOp::And),
            BinOpKind::Or => Ok(mir::BinOp::Or),
            BinOpKind::BitAnd => Ok(mir::BinOp::BitAnd),
            BinOpKind::BitOr => Ok(mir::BinOp::BitOr),
            BinOpKind::BitXor => Ok(mir::BinOp::BitXor),
    ...
    pub(super) fn convert_thir_binop_to_kind(&self, op: thir::BinOp) -> Result<BinOpKind> {
        match op {
            ...
            thir::BinOp::Rem => Ok(BinOpKind::Mod),
            thir::BinOp::And => Ok(BinOpKind::And),
            thir::BinOp::Or => Ok(BinOpKind::Or),
            ...
            thir::BinOp::BitAnd => Ok(BinOpKind::BitAnd),
            thir::BinOp::BitOr => Ok(BinOpKind::BitOr),
            thir::BinOp::BitXor => Ok(BinOpKind::BitXor),
            other => Err(optimization_error(format!(
                "Unsupported THIR binary operator {:?} during MIR lowering",
                other
            ))),
