; ModuleID = '15_enums'
source_filename = "15_enums"
target datalayout = "e-m:o-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-n32:64-S128-Fn32"
target triple = "arm64-apple-darwin25.0.0"

@global_5 = internal constant [0 x { { ptr, i64 }, ptr }] zeroinitializer, align 8
@.str.15_enums.0 = constant [51 x i8] c"test result: %lld passed; %lld failed; %lld total\0A\00"
@.str.15_enums.1 = constant [13 x i8] c"  %s ... ok\0A\00"
@.str.15_enums.2 = constant [17 x i8] c"  %s ... FAILED\0A\00"
@.str.15_enums.3 = constant [6 x i8] c"point\00"
@.str.15_enums.4 = constant [7 x i8] c"circle\00"
@.str.15_enums.5 = constant [10 x i8] c"rectangle\00"
@.str.15_enums.6 = constant [28 x i8] c"\F0\9F\93\98 Tutorial: 15_enums.fp\0A\00"
@.str.15_enums.7 = constant [75 x i8] c"\F0\9F\A7\AD Focus: Enum variants: unit, tuple, struct variants and discriminants\0A\00"
@.str.15_enums.8 = constant [46 x i8] c"\F0\9F\A7\AA What to look for: labeled outputs below\0A\00"
@.str.15_enums.9 = constant [39 x i8] c"\E2\9C\85 Expectation: outputs match labels\0A\00"
@.str.15_enums.10 = constant [2 x i8] c"\0A\00"
@.str.15_enums.11 = constant [19 x i8] c"shape point -> %s\0A\00"
@.str.15_enums.12 = constant [20 x i8] c"shape circle -> %s\0A\00"
@.str.15_enums.13 = constant [23 x i8] c"shape rectangle -> %s\0A\00"
@.str.15_enums.14 = constant [18 x i8] c"discriminant: %d\0A\00"
@.str.15_enums.15 = constant [31 x i8] c"unwrap_or(Some(42), 0) = %lld\0A\00"
@.str.15_enums.16 = constant [28 x i8] c"unwrap_or(None, 99) = %lld\0A\00"
@.str.15_enums.17 = constant [11 x i8] c"const: %d\0A\00"

define internal void @__closure0_call({ ptr } %0) {
bb0:
  %alloca_0 = alloca ptr, align 8
  %alloca_count_0 = alloca ptr, align 8
  store { ptr } %0, ptr %alloca_count_0, align 8
  %load_3 = load ptr, ptr %alloca_count_0, align 8
  call void @TestCase__run(ptr %load_3)
  br label %bb1

bb1:                                              ; preds = %bb0
  ret void
}

define internal { i64, i64, i64 } @std__test__run_tests() personality ptr @__gxx_personality_v0 {
bb0:
  %alloca_68 = alloca i1, align 1
  %alloca_count_68 = alloca i1, align 1
  %alloca_50 = alloca i64, align 8
  %alloca_count_50 = alloca i64, align 8
  %alloca_45 = alloca i64, align 8
  %alloca_count_45 = alloca i64, align 8
  %alloca_35 = alloca { { ptr, i64 }, ptr }, align 8
  %alloca_count_35 = alloca { { ptr, i64 }, ptr }, align 8
  %alloca_32 = alloca i64, align 8
  %alloca_count_32 = alloca i64, align 8
  %alloca_29 = alloca i64, align 8
  %alloca_count_29 = alloca i64, align 8
  %alloca_23 = alloca i1, align 1
  %alloca_count_23 = alloca i1, align 1
  %alloca_21 = alloca i64, align 8
  %alloca_count_21 = alloca i64, align 8
  %alloca_15 = alloca [0 x { { ptr, i64 }, ptr }], align 8
  %alloca_count_15 = alloca [0 x { { ptr, i64 }, ptr }], align 8
  %alloca_13 = alloca [0 x { { ptr, i64 }, ptr }], align 8
  %alloca_count_13 = alloca [0 x { { ptr, i64 }, ptr }], align 8
  %alloca_11 = alloca [0 x { { ptr, i64 }, ptr }], align 8
  %alloca_count_11 = alloca [0 x { { ptr, i64 }, ptr }], align 8
  %alloca_10 = alloca i64, align 8
  %alloca_count_10 = alloca i64, align 8
  %alloca_9 = alloca i64, align 8
  %alloca_count_9 = alloca i64, align 8
  %alloca_8 = alloca i64, align 8
  %alloca_count_8 = alloca i64, align 8
  %alloca_7 = alloca i1, align 1
  %alloca_count_7 = alloca i1, align 1
  %alloca_6 = alloca { i64, i64, i64 }, align 8
  %alloca_count_6 = alloca { i64, i64, i64 }, align 8
  store [0 x { { ptr, i64 }, ptr }] zeroinitializer, ptr %alloca_count_11, align 8
  store [0 x { { ptr, i64 }, ptr }] zeroinitializer, ptr %alloca_count_13, align 8
  %load_16 = load [0 x { { ptr, i64 }, ptr }], ptr %alloca_count_13, align 8
  store [0 x { { ptr, i64 }, ptr }] %load_16, ptr %alloca_count_15, align 8
  store i64 0, ptr %alloca_count_8, align 8
  store i64 0, ptr %alloca_count_9, align 8
  store i64 0, ptr %alloca_count_10, align 8
  br label %bb1

bb1:                                              ; preds = %bb9, %bb0
  store i64 0, ptr %alloca_count_21, align 8
  %load_24 = load i64, ptr %alloca_count_10, align 8
  %load_25 = load i64, ptr %alloca_count_21, align 8
  %icmp_26 = icmp slt i64 %load_24, %load_25
  store i1 %icmp_26, ptr %alloca_count_23, align 1
  %load_28 = load i1, ptr %alloca_count_23, align 1
  br i1 %load_28, label %bb2, label %bb3

bb2:                                              ; preds = %bb1
  %load_30 = load i64, ptr %alloca_count_10, align 8
  store i64 %load_30, ptr %alloca_count_29, align 8
  %load_33 = load i64, ptr %alloca_count_10, align 8
  store i64 %load_33, ptr %alloca_count_32, align 8
  %load_36 = load i64, ptr %alloca_count_32, align 8
  %iop_37 = mul i64 %load_36, 24
  %gep_39 = getelementptr inbounds i8, ptr %alloca_count_15, i64 %iop_37
  %load_41 = load { { ptr, i64 }, ptr }, ptr %gep_39, align 8
  store { { ptr, i64 }, ptr } %load_41, ptr %alloca_count_35, align 8
  %load_43 = load { { ptr, i64 }, ptr }, ptr %alloca_count_35, align 8
  invoke void @__closure0_call({ ptr } undef)
          to label %bb4 unwind label %bb5

bb3:                                              ; preds = %bb1
  %load_46 = load i64, ptr %alloca_count_8, align 8
  %load_47 = load i64, ptr %alloca_count_9, align 8
  %iop_48 = add i64 %load_46, %load_47
  store i64 %iop_48, ptr %alloca_count_45, align 8
  %load_51 = load i64, ptr %alloca_count_45, align 8
  store i64 %load_51, ptr %alloca_count_50, align 8
  %load_53 = load i64, ptr %alloca_count_8, align 8
  %load_54 = load i64, ptr %alloca_count_9, align 8
  %load_55 = load i64, ptr %alloca_count_50, align 8
  %call_56 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.0, i64 %load_53, i64 %load_54, i64 %load_55)
  br label %bb12

bb4:                                              ; preds = %bb2
  store i1 true, ptr %alloca_count_7, align 1
  br label %bb6

bb5:                                              ; preds = %bb2
  %landingpad = landingpad { ptr, i32 }
          catch ptr null
  store i1 false, ptr %alloca_count_7, align 1
  br label %bb6

bb12:                                             ; preds = %bb3
  %load_60 = load i64, ptr %alloca_count_50, align 8
  %load_61 = load i64, ptr %alloca_count_8, align 8
  %load_62 = load i64, ptr %alloca_count_9, align 8
  %insertvalue_63 = insertvalue { i64, i64, i64 } undef, i64 %load_60, 0
  %insertvalue_64 = insertvalue { i64, i64, i64 } %insertvalue_63, i64 %load_61, 1
  %insertvalue_65 = insertvalue { i64, i64, i64 } %insertvalue_64, i64 %load_62, 2
  store { i64, i64, i64 } %insertvalue_65, ptr %alloca_count_6, align 8
  %load_67 = load { i64, i64, i64 }, ptr %alloca_count_6, align 8
  ret { i64, i64, i64 } %load_67

bb6:                                              ; preds = %bb5, %bb4
  %load_69 = load i1, ptr %alloca_count_7, align 1
  store i1 %load_69, ptr %alloca_count_68, align 1
  %load_71 = load i1, ptr %alloca_count_68, align 1
  br i1 %load_71, label %bb7, label %bb8

bb7:                                              ; preds = %bb6
  %load_72 = load i64, ptr %alloca_count_8, align 8
  %iop_73 = add i64 %load_72, 1
  store i64 %iop_73, ptr %alloca_count_8, align 8
  %load_76 = load { ptr, i64 }, ptr %alloca_count_35, align 8
  %call_77 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.1, { ptr, i64 } %load_76)
  br label %bb10

bb8:                                              ; preds = %bb6
  %load_78 = load i64, ptr %alloca_count_9, align 8
  %iop_79 = add i64 %load_78, 1
  store i64 %iop_79, ptr %alloca_count_9, align 8
  %load_82 = load { ptr, i64 }, ptr %alloca_count_35, align 8
  %call_83 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.2, { ptr, i64 } %load_82)
  br label %bb11

bb10:                                             ; preds = %bb7
  br label %bb9

bb11:                                             ; preds = %bb8
  br label %bb9

bb9:                                              ; preds = %bb11, %bb10
  %load_84 = load i64, ptr %alloca_count_10, align 8
  %iop_85 = add i64 %load_84, 1
  store i64 %iop_85, ptr %alloca_count_10, align 8
  br label %bb1
}

define internal { i64, i64, i64 } @std__test__run() {
bb0:
  %alloca_87 = alloca { i64, i64, i64 }, align 8
  %alloca_count_87 = alloca { i64, i64, i64 }, align 8
  %call_88 = call { i64, i64, i64 } @std__test__run_tests()
  store { i64, i64, i64 } %call_88, ptr %alloca_count_87, align 8
  br label %bb1

bb1:                                              ; preds = %bb0
  %load_90 = load { i64, i64, i64 }, ptr %alloca_count_87, align 8
  ret { i64, i64, i64 } %load_90
}

define internal ptr @Shape__describe(ptr %0) {
bb0:
  %alloca_111 = alloca i1, align 1
  %alloca_count_111 = alloca i1, align 1
  %alloca_102 = alloca i1, align 1
  %alloca_count_102 = alloca i1, align 1
  %alloca_94 = alloca i1, align 1
  %alloca_count_94 = alloca i1, align 1
  %alloca_92 = alloca ptr, align 8
  %alloca_count_92 = alloca ptr, align 8
  %alloca_91 = alloca ptr, align 8
  %alloca_count_91 = alloca ptr, align 8
  store ptr %0, ptr %alloca_count_92, align 8
  %load_95 = load ptr, ptr %alloca_count_92, align 8
  %load_97 = load i64, ptr %load_95, align 8
  %icmp_98 = icmp eq i64 %load_97, 0
  store i1 %icmp_98, ptr %alloca_count_94, align 1
  %load_100 = load i1, ptr %alloca_count_94, align 1
  br i1 %load_100, label %bb2, label %bb3

bb2:                                              ; preds = %bb0
  store ptr @.str.15_enums.3, ptr %alloca_count_91, align 8
  br label %bb1

bb3:                                              ; preds = %bb0
  %load_103 = load ptr, ptr %alloca_count_92, align 8
  %load_105 = load i64, ptr %load_103, align 8
  %icmp_106 = icmp eq i64 %load_105, 1
  store i1 %icmp_106, ptr %alloca_count_102, align 1
  %load_108 = load i1, ptr %alloca_count_102, align 1
  br i1 %load_108, label %bb4, label %bb5

bb1:                                              ; preds = %bb7, %bb6, %bb4, %bb2
  %load_109 = load ptr, ptr %alloca_count_91, align 8
  ret ptr %load_109

bb4:                                              ; preds = %bb3
  store ptr @.str.15_enums.4, ptr %alloca_count_91, align 8
  br label %bb1

bb5:                                              ; preds = %bb3
  %load_112 = load ptr, ptr %alloca_count_92, align 8
  %load_114 = load i64, ptr %load_112, align 8
  %icmp_115 = icmp eq i64 %load_114, 2
  store i1 %icmp_115, ptr %alloca_count_111, align 1
  %load_117 = load i1, ptr %alloca_count_111, align 1
  br i1 %load_117, label %bb6, label %bb7

bb6:                                              ; preds = %bb5
  store ptr @.str.15_enums.5, ptr %alloca_count_91, align 8
  br label %bb1

bb7:                                              ; preds = %bb5
  br label %bb1
}

define i32 @main() {
bb0:
  %alloca_172 = alloca i32, align 4
  %alloca_count_172 = alloca i32, align 4
  %alloca_163 = alloca { i64, i64 }, align 8
  %alloca_count_163 = alloca { i64, i64 }, align 8
  %alloca_161 = alloca { i64, i64 }, align 8
  %alloca_count_161 = alloca { i64, i64 }, align 8
  %alloca_159 = alloca { i64, i64 }, align 8
  %alloca_count_159 = alloca { i64, i64 }, align 8
  %alloca_153 = alloca i32, align 4
  %alloca_count_153 = alloca i32, align 4
  %alloca_150 = alloca i64, align 8
  %alloca_count_150 = alloca i64, align 8
  %alloca_148 = alloca i64, align 8
  %alloca_count_148 = alloca i64, align 8
  %alloca_143 = alloca ptr, align 8
  %alloca_count_143 = alloca ptr, align 8
  %alloca_138 = alloca ptr, align 8
  %alloca_count_138 = alloca ptr, align 8
  %alloca_133 = alloca ptr, align 8
  %alloca_count_133 = alloca ptr, align 8
  %alloca_131 = alloca { i64, i64, i64 }, align 8
  %alloca_count_131 = alloca { i64, i64, i64 }, align 8
  %alloca_129 = alloca { i64, i64, i64 }, align 8
  %alloca_count_129 = alloca { i64, i64, i64 }, align 8
  %alloca_126 = alloca { i64, i64, i64 }, align 8
  %alloca_count_126 = alloca { i64, i64, i64 }, align 8
  %alloca_124 = alloca { i64, i64, i64 }, align 8
  %alloca_count_124 = alloca { i64, i64, i64 }, align 8
  %call_119 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.6)
  br label %bb1

bb1:                                              ; preds = %bb0
  %call_120 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.7)
  br label %bb2

bb2:                                              ; preds = %bb1
  %call_121 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.8)
  br label %bb3

bb3:                                              ; preds = %bb2
  %call_122 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.9)
  br label %bb4

bb4:                                              ; preds = %bb3
  %call_123 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.10)
  br label %bb5

bb5:                                              ; preds = %bb4
  store { i64, i64, i64 } zeroinitializer, ptr %alloca_count_124, align 8
  %load_127 = load { i64, i64, i64 }, ptr %alloca_count_124, align 8
  store { i64, i64, i64 } %load_127, ptr %alloca_count_126, align 8
  store { i64, i64, i64 } { i64 1, i64 10, i64 0 }, ptr %alloca_count_129, align 8
  store { i64, i64, i64 } { i64 2, i64 5, i64 3 }, ptr %alloca_count_131, align 8
  store ptr %alloca_count_126, ptr %alloca_count_133, align 8
  %load_135 = load ptr, ptr %alloca_count_133, align 8
  %call_136 = call ptr @Shape__describe(ptr %load_135)
  br label %bb6

bb6:                                              ; preds = %bb5
  %call_137 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.11, ptr %call_136)
  br label %bb7

bb7:                                              ; preds = %bb6
  store ptr %alloca_count_129, ptr %alloca_count_138, align 8
  %load_140 = load ptr, ptr %alloca_count_138, align 8
  %call_141 = call ptr @Shape__describe(ptr %load_140)
  br label %bb8

bb8:                                              ; preds = %bb7
  %call_142 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.12, ptr %call_141)
  br label %bb9

bb9:                                              ; preds = %bb8
  store ptr %alloca_count_131, ptr %alloca_count_143, align 8
  %load_145 = load ptr, ptr %alloca_count_143, align 8
  %call_146 = call ptr @Shape__describe(ptr %load_145)
  br label %bb10

bb10:                                             ; preds = %bb9
  %call_147 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.13, ptr %call_146)
  br label %bb11

bb11:                                             ; preds = %bb10
  store i64 5, ptr %alloca_count_148, align 8
  %load_151 = load i64, ptr %alloca_count_148, align 8
  store i64 %load_151, ptr %alloca_count_150, align 8
  %load_154 = load i64, ptr %alloca_count_150, align 8
  %trunc = trunc i64 %load_154 to i32
  store i32 %trunc, ptr %alloca_count_153, align 4
  %load_157 = load i32, ptr %alloca_count_153, align 4
  %call_158 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.14, i32 %load_157)
  br label %bb12

bb12:                                             ; preds = %bb11
  store { i64, i64 } { i64 0, i64 42 }, ptr %alloca_count_159, align 8
  store { i64, i64 } { i64 1, i64 0 }, ptr %alloca_count_161, align 8
  %load_164 = load { i64, i64 }, ptr %alloca_count_161, align 8
  store { i64, i64 } %load_164, ptr %alloca_count_163, align 8
  %load_166 = load { i64, i64 }, ptr %alloca_count_159, align 8
  %call_167 = call i64 @Option__unwrap_or__mono_a7af9f593fdc4675({ i64, i64 } %load_166, i64 0)
  br label %bb13

bb13:                                             ; preds = %bb12
  %call_168 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.15, i64 %call_167)
  %load_169 = load { i64, i64 }, ptr %alloca_count_163, align 8
  %call_170 = call i64 @Option__unwrap_or__mono_a7af9f593fdc4675({ i64, i64 } %load_169, i64 99)
  br label %bb14

bb14:                                             ; preds = %bb13
  %call_171 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.16, i64 %call_170)
  store i64 2, ptr %alloca_count_172, align 4
  %load_174 = load i32, ptr %alloca_count_172, align 4
  %call_175 = call i32 (ptr, ...) @printf(ptr @.str.15_enums.17, i32 %load_174)
  br label %bb15

bb15:                                             ; preds = %bb14
  ret i32 0
}

define internal i64 @Option__unwrap_or__mono_a7af9f593fdc4675({ i64, i64 } %0, i64 %1) {
bb0:
  %alloca_193 = alloca i1, align 1
  %alloca_count_193 = alloca i1, align 1
  %alloca_185 = alloca i64, align 8
  %alloca_count_185 = alloca i64, align 8
  %alloca_179 = alloca i1, align 1
  %alloca_count_179 = alloca i1, align 1
  %alloca_177 = alloca { i64, i64 }, align 8
  %alloca_count_177 = alloca { i64, i64 }, align 8
  %alloca_176 = alloca i64, align 8
  %alloca_count_176 = alloca i64, align 8
  store { i64, i64 } %0, ptr %alloca_count_177, align 8
  %load_181 = load i64, ptr %alloca_count_177, align 8
  %icmp_182 = icmp eq i64 %load_181, 0
  store i1 %icmp_182, ptr %alloca_count_179, align 1
  %load_184 = load i1, ptr %alloca_count_179, align 1
  br i1 %load_184, label %bb2, label %bb3

bb2:                                              ; preds = %bb0
  %gep_187 = getelementptr inbounds i8, ptr %alloca_count_177, i64 8
  %load_189 = load i64, ptr %gep_187, align 8
  store i64 %load_189, ptr %alloca_count_185, align 8
  %load_191 = load i64, ptr %alloca_count_185, align 8
  store i64 %load_191, ptr %alloca_count_176, align 8
  br label %bb1

bb3:                                              ; preds = %bb0
  %load_195 = load i64, ptr %alloca_count_177, align 8
  %icmp_196 = icmp eq i64 %load_195, 1
  store i1 %icmp_196, ptr %alloca_count_193, align 1
  %load_198 = load i1, ptr %alloca_count_193, align 1
  br i1 %load_198, label %bb4, label %bb5

bb1:                                              ; preds = %bb5, %bb4, %bb2
  %load_199 = load i64, ptr %alloca_count_176, align 8
  ret i64 %load_199

bb4:                                              ; preds = %bb3
  store i64 %1, ptr %alloca_count_176, align 8
  br label %bb1

bb5:                                              ; preds = %bb3
  br label %bb1
}

define internal void @TestCase__run(ptr %0) {
bb0:
  ret void
}

declare i32 @printf(ptr, ...)

declare i32 @__gxx_personality_v0(...)
