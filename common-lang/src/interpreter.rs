...
impl InterpreterPass {
    ...
    pub fn interpret_stmt(&self, node: &Statement, ctx: &ScopedContext) -> Result<Option<Value>> {
...
impl OptimizePass for InterpreterPass {
    ...
    fn optimize_item_pre(&self, item: Item, _ctx: &ScopedContext) -> Result<Option<Item>> {
        match item {
