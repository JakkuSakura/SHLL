...
fn execute(options: &TestOptions) -> crate::Result<()> {
    ...
    let output_root = root.join("target").join(&profile).join("magnet").join("test");
    ...
    let runner = output_dir.join("test_main.fp");
    std::fs::write(&runner, test_runner_source())?;

    let mut sources = collect_sources(&package.root_path.join("src"), &["fp"])?;
    sources.extend(collect_sources(&package.root_path.join("tests"), &["fp"])?) ;
    sources.push(runner.clone());
    ...
    let output = output_path_for_entry(&runner, &output_dir);
    ...
    if !status.success() {
        return Err("test binary failed".to_string().into());
    }
    Ok(())
}
...
fn test_runner_source() -> &'static str {
    r#"fn main() {
