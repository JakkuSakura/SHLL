...
static GLOBAL_DIAGNOSTIC_RENDERER: OnceLock<Mutex<Option<DiagnosticRenderer>>> = OnceLock::new();
static EMIT_TRACING: OnceLock<AtomicBool> = OnceLock::new();

pub fn set_diagnostic_renderer(renderer: DiagnosticRenderer) {
    if let Ok(mut guard) = GLOBAL_DIAGNOSTIC_RENDERER
        .get_or_init(|| Mutex::new(None))
        .lock()
    {
        *guard = Some(renderer);
...
static GLOBAL_DIAGNOSTIC_MANAGER: OnceLock<Arc<DiagnosticManager>> = OnceLock::new();

pub fn diagnostic_manager() -> Arc<DiagnosticManager> {
    GLOBAL_DIAGNOSTIC_MANAGER
        .get_or_init(|| Arc::new(DiagnosticManager::new()))
        .clone()
}
...
fn emit_tracing(level: &DiagnosticLevel, context: Option<&str>, message: &str) {
    ...
    match level {
        DiagnosticLevel::Error => eprintln!("[error] {}", msg),
        DiagnosticLevel::Warning => eprintln!("[warn] {}", msg),
        DiagnosticLevel::Info => eprintln!("[info] {}", msg),
    }
}
