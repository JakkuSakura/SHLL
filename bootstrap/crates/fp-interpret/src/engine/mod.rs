...
impl<'ctx> AstInterpreter<'ctx> {
    ...
    fn item_to_token_stream(&self, item: &Item) -> Result<Vec<MacroTokenTree>> {
        ...
        Ok(macro_token_trees_from_text(&text))
    }
...
fn macro_token_trees_from_text(text: &str) -> Vec<MacroTokenTree> {
    let mut chars = text.char_indices().peekable();
    macro_token_trees_from_chars(&mut chars, None)
}

fn macro_token_trees_from_chars(
    chars: &mut std::iter::Peekable<std::str::CharIndices<'_>>,
    stop: Option<char>,
) -> Vec<MacroTokenTree> {
    let mut tokens = Vec::new();
    while let Some((_, ch)) = chars.next() {
        if let Some(stop_ch) = stop {
            if ch == stop_ch {
                break;
            }
        }
        ...
        match ch {
            '(' | '{' | '[' => {
                let (close, delimiter) = match ch {
                    '(' => (')', fp_core::ast::MacroDelimiter::Parenthesis),
                    '{' => ('}', fp_core::ast::MacroDelimiter::Brace),
                    '[' => (']', fp_core::ast::MacroDelimiter::Bracket),
                    _ => unreachable!(),
                };
                let inner = macro_token_trees_from_chars(chars, Some(close));
                tokens.push(MacroTokenTree::Group(MacroGroup {
                    delimiter,
                    tokens: inner,
                    span: Span::null(),
                }));
            ...
            ')' | '}' | ']' => {
                push_token(&mut tokens, ch.to_string());
            }
            '"' | '\'' => {
                let literal = read_quoted_literal(ch, chars);
                push_token(&mut tokens, literal);
            }
            'r' => {
                if let Some(literal) = read_raw_string(chars) {
                    push_token(&mut tokens, literal);
                } else {
                    let token = read_token(ch, chars);
...
fn push_token(tokens: &mut Vec<MacroTokenTree>, text: String) {
    if text.is_empty() {
        return;
    }
    tokens.push(MacroTokenTree::Token(MacroToken {
        text,
        span: Span::null(),
    }));
}
...
fn read_raw_string(chars: &mut std::iter::Peekable<std::str::CharIndices<'_>>) -> Option<String> {
