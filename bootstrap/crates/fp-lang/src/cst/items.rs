#[allow(deprecated)] // ErrorKind required by legacy error API.
use fp_core::winnow::error::{ContextError, ErrMode, ErrorKind, FromExternalError};
...
impl From<LexerError> for ItemParseError {
    fn from(err: LexerError) -> Self {
        ItemParseError::Lex(err)
    }
}
...
impl std::fmt::Display for ItemParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ItemParseError::Lex(err) => write!(f, "lex error: {}", err),
            ItemParseError::Parse { message, .. } => write!(f, "parse error: {}", message),
        }
    }
}
