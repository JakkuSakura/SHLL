...
impl std::fmt::Display for LowerItemsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            LowerItemsError::UnexpectedNode(kind) => write!(f, "unexpected CST node kind: {:?}", kind),
            LowerItemsError::MissingToken(token) => write!(f, "missing required token: {}", token),
            LowerItemsError::UnsupportedAbi(abi) => write!(f, "unsupported extern ABI: {}", abi),
        }
    }
}

impl std::error::Error for LowerItemsError {}
            ...
            Ty::Expr(expr) => match expr.kind() {
                ExprKind::Locator(locator) => locator.as_ident().and_then(|id| match id.as_str() {
                    ...
                    | "statics" | "mods" | "uses" | "macros" => {
                        fp_core::warn!("deprecated plural quote fragment kind: {}", id.as_str());
