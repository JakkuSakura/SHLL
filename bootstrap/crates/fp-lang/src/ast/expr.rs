...
pub enum LowerError {
    ...
    Unsupported(String),
    Item(LowerItemsError),
}

impl std::fmt::Display for LowerError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            LowerError::UnexpectedNode(kind) => write!(f, "unexpected CST node kind: {:?}", kind),
            LowerError::MissingOperator => write!(f, "missing operator token"),
            LowerError::InvalidNumber(value) => write!(f, "invalid number literal: {}", value),
            LowerError::Unsupported(feature) => write!(f, "unsupported feature: {}", feature),
            LowerError::Item(err) => write!(f, "failed to lower item: {}", err),
        }
    }
}
...
impl From<LowerItemsError> for LowerError {
    fn from(err: LowerItemsError) -> Self {
        LowerError::Item(err)
    }
}
...
fn quote_kind_from_cst(node: &SyntaxNode) -> Result<Option<QuoteFragmentKind>, LowerError> {
    ...
    while let Some(tok) = iter.next() {
        ...
        let kind = match kind_tok.text.as_str() {
            ...
            | "mods" | "uses" | "macros" => {
                fp_core::warn!("deprecated plural quote fragment kind: {}", kind_tok.text);
...
pub fn lower_expr_from_cst(node: &SyntaxNode) -> Result<Expr, LowerError> {
    let mut expr = match node.kind {
        ...
        SyntaxKind::ExprPath => {
            ...
            Ok(ExprKind::Name(Name::path(Path::new(segments))).into())
        }
...
fn parse_numeric_literal(raw: &str) -> Result<Value, LowerError> {
    ...
    match suffix {
        "ib" | "fb" => Err(LowerError::InvalidNumber(raw.to_string())),
        _ => {
