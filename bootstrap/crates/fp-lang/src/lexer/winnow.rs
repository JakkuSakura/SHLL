use fp_core::winnow::combinator::{alt, cut_err, opt, repeat};
use fp_core::winnow::error::{ContextError, ErrMode};
use fp_core::winnow::token::{literal, take_till, take_until, take_while};
use fp_core::winnow::{ModalResult, Parser};
...
pub(crate) fn ws(input: &mut &str) -> ModalResult<()> {
    repeat(0.., alt((whitespace, line_comment, block_comment)))
        .parse_next(input)?;
    Ok(())
}
